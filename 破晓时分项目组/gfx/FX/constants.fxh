...
static const float GB_CAM_MAX_FILLING_CLAMP = 1.1f; // 0 to 1 value for clamping the fill when camera is at max distance
...
static const float BORDER_FOW_REMOVAL_FACTOR = 0.01f; // How much of the FOW that is removed from the borders. 1.0f is no FOW
static const float BORDER_LIGHT_REMOVAL_FACTOR = 0.1f; // 国家边界上移除的光亮，1.0f就是没有光。也就是说越大，边界的光越多（实测后发现还和全局光相关，如果调高的话低地（比如峡谷和海岸）的亮度就会上升，如果调低了话高低（比如山脉）的亮度就会上升）（调成0.1就会有类似冷战地图的效果）。！！！绝对不能调1.5以上！！！电脑会崩溃！！！
static const float GB_STRENGTH_CH1 = 1.0; //底层颜色的不透明度（和BORDER_LIGHT_REMOVAL_FACTOR类似，基础是1）
static const float GB_STRENGTH_CH2 = 0.1; // 顶层颜色的不透明度（有点像是全局光的感觉，越高光线越暗）（该选项还会和GB_OPACITY_FAR相关联。在GB_OPACITY_FAR高数值的情况下同等数值的GB_STRENGTH_CH2会更深）
static const float GB_FIRST_LAYER_PRIORITY = 1.0; // Priority for first/second layer when both are active at the same pixel
